...
function validateHeaderValue(name: string, value: string) {
    ...
    value.split(/\n/g).forEach((key, lineIx) => {
        ...
        const headerValues = headerValue.split(',').map((v) => v.trim()).filter(Boolean);
...
export const HeaderOptionsField = (props:Props) => {
        return (
            <>
                <TextAreaWithLines label={'Headers'}
                                   name={props.name}
                                   validation={[validateHeaderValue]}/>
                <pre style={{
                    color: grey[600],
                    fontSize: '12px'
                }}>
                    Define the headers to be used when publishing to a headers exchange.<br/><br/>
                    <b>Format:</b><br/>
                    Some-Header: value1, value2, value3<br/>
